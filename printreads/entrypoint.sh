...
VERSION="0.1.0"
GATK_THREADS=1
# Entrypoint script for docker container buddej/printreads:${VERSION}
...
#   BAMFILE     .bam input file on which to perform Print Reads
#   BQSR        .table file generated by GATK BaseRecalibrator
...
if [ -z "${BQSR}" ]; then
    echo "ERROR, must provide .table file in variable \${BQSR}"
    quit "Input File"
fi
if ! [ -r "${BAMFILE}" ]; then
